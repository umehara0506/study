...
class Expression(ABC):
    ...
    @abstractmethod
    def times(self, multiplier: int) -> Exception:
        pass
    
    @abstractmethod
    def plus(self, to: str) -> Exception:
        pass
    
    @abstractmethod
    def reduce(self, to: str) -> Exception:
        pass
...
class MoneyTest:
    ...
    def test_sum_plus_moeny():
        ...
        bank = Bank()
        bank.addRate('CHF', 'USD', 2)
        sum: Exception = Sum(fiveBuckes, tenFrances).plus(fiveBuckes)
    ...
    def test_sum_times():
        ...
        bank = Bank()
        bank.addRate('CHF', 'USD', 2)
        sum: Exception = Sum(fiveBuckes, tenFrances).times(2)
        result: Money = bank.reduce(sum, 'USD')
...
MoneyTest.test_sum_plus_moeny()
MoneyTest.test_sum_times()